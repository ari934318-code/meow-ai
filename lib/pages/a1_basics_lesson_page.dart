...
class _A1BasicsLessonPageState
  ...
  _A1Stage _listeningStage(String title, String titleFa) {
    return _A1Stage(
      title: title,
      titleFa: titleFa,
      description: 'Listen carefully, then choose the sentence you hear.',
      descriptionFa: 'با دقت گوش بده، سپس جمله‌ای را که می‌شنوی انتخاب کن.',
      isListening: true,
    );
  }
  ...
  Widget _buildListeningContent() {
    if (_listeningQuestions.isEmpty) {
      return Text(_isPersian ? 'محتوای شنیداری این درس آماده نیست.' : 'Listening content is not available yet.');
    }
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: List.generate(_listeningQuestions.length, (index) {
        ...
        return Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: !answered ? _outline : correct ? Colors.green.withAlpha(55) : Colors.red.withAlpha(45)),
          ),
          child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            Text(_isPersian ? item.sentenceFa : item.sentence, style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
            const SizedBox(height: 12),
            OutlinedButton.icon(onPressed: () => _speak(item.sentence), icon: const Icon(Icons.volume_up_rounded), label: Text(_isPersian ? 'پخش جمله' : 'Play sentence')),
            const SizedBox(height: 12),
            ...
            if (answered)
              Text(
                correct ? (_isPersian ? 'درست! 😼🎧' : 'Correct! 😼🎧') : (_isPersian ? 'جواب درست: '+item.answer : 'Correct answer: '+item.answer),
  ...
  Future<void> _answerListening(int index, String answer) async {
    if (_listeningAnswered.contains(index)) return;
    setState(() {
  ...
  bool _isCurrentStageComplete() {
    ...
    if (stage.isListening) {
      return _listeningQuestions.isEmpty ||
          List.generate(_listeningQuestions.length, (i) => i).every(_listeningAnswered.contains);
    }
    final questionsComplete = stage.questionIndices.every(_answeredQuestions.contains);
    final speakingComplete = stage.speakingIndices.every(_completedSpeaking.contains);
    return questionsComplete && speakingComplete;
  }
  ...
  Widget _buildCurrentStage() {
    if (_stages.isEmpty) {
      return Center(child: Text(_isPersian ? 'این درس هنوز محتوایی ندارد.' : 'This lesson has no content yet.'));
    }
    final stage = _stages[_currentStage];
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        Container(
          ...
          decoration: BoxDecoration(color: _surface, borderRadius: BorderRadius.circular(24), border: Border.all(color: _outline)),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(_isPersian ? stage.titleFa : stage.title, style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)),
            const SizedBox(height: 6),
            Text(_isPersian ? stage.descriptionFa : stage.description, style: Theme.of(context).textTheme.bodyLarge?.copyWith(height: 1.45)),
          ]),
        ),
        SizedBox(height: A1BasicsUIConfig.sectionSpacing),
        if (_learningMode)
          _buildLearningContent()
        else if (stage.isListening)
        ...
        if (!_learningMode) ...[
          ...
          FilledButton(
            ...
            style: FilledButton.styleFrom(backgroundColor: lavender, foregroundColor: Colors.black87, padding: const EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(A1BasicsUIConfig.buttonRadius))),
            child: Text(_currentStage == _stages.length - 1 ? (_isPersian ? 'پایان درس' : 'Finish Lesson') : (_isPersian ? 'تکمیل مرحله' : 'Complete Stage')),
          ),
        ],
      ],
    );
