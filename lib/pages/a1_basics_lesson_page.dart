...
class _A1BasicsLessonPageState
  ...
  List<_A1Stage> _buildStages() {
    switch (widget.lesson.id) {
      ...
      case '1':
        return [
          _stage('Stage 1', 'مرحله ۱', [0, 1, 2, 3, 4]),
          _stage('Stage 2', 'مرحله ۲', [5, 6, 7, 8, 9]),
          _stage('Stage 3', 'مرحله ۳', [10, 11, 12, 13, 14]),
          _stage('Stage 4', 'مرحله ۴', [15, 16, 17, 18, 19]),
          _stage('Stage 5', 'مرحله ۵', [20, 21, 22, 23, 24]),
          _stage('Stage 6', 'مرحله ۶', [25, 26, 27, 28, 29]),
          _stage('Stage 7', 'مرحله ۷', [30, 31, 32, 33, 34]),
          _speakingStage(
            'Speaking',
            'تمرین مکالمه',
          ),
      ...
      case '12':
        return [
          ...
          _stage('Guided Practice', 'تمرین هدایت‌شده', List.generate(30, (i) => i)),
          _stage('Independent Production', 'تولید مستقل', List.generate(30, (i) => i)),
          _stage('Real-Life Use', 'کاربرد واقعی', List.generate(30, (i) => i)),
      ...
      case '14':
        return [
          ...
          _stage('Key Time Expressions', 'عبارت‌های زمانی مهم', List.generate(37, (i) => i)),
          _stage('Guided Practice', 'تمرین هدایت‌شده', List.generate(37, (i) => i)),
          _stage('Independent Production', 'تولید مستقل', List.generate(37, (i) => i)),
          _stage('Real-World Use', 'کاربرد واقعی', List.generate(37, (i) => i)),
          _listeningStage('Listening', 'گوش دادن'),
          _speakingStage('Speaking', 'تمرین تلفظ و مکالمه'),
        ];
      ...
      case '15':
        return [
          ...
          _stage('Guided Practice', 'تمرین هدایت‌شده', List.generate(36, (i) => i)),
          _stage('Independent Production', 'تولید مستقل', List.generate(36, (i) => i)),
          _stage('Real-World Use', 'کاربرد واقعی', List.generate(36, (i) => i)),
          _listeningStage('Listening', 'گوش دادن'),
          _speakingStage('Speaking', 'تمرین تلفظ و مکالمه'),
        ];
  ...
  _A1Stage _stage(
    ...
    return _A1Stage(
      ...
      // Keep stage definitions data-driven, but never allow a stale
      // question index to crash the lesson after a question-bank change.
      questionIndices: questions
          .where((index) => index >= 0 && index < _questions.length)
          .toList(),
    );
  }
